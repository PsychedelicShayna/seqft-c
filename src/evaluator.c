...
double eval_binary_op(tokent_t operator_type, double num1, double num2) {
  ...
  else if(operator_type == TT_OP_FLOOR) {
    return floor(num1 / num2);
  }

  // Bitwise operations.
  else if(operator_type == TT_OP_XOR) {
    uint64_t uint1 = (uint64_t)num1, uint2 = (uint64_t)num2;
    uint64_t result = uint1 ^ uint2;
    return (double)result;
  } else if(operator_type == TT_OP_OR) {
    uint64_t uint1 = (uint64_t)num1, uint2 = (uint64_t)num2;
    uint64_t result = uint1 | uint2;
    return (double)result;
  } else if(operator_type == TT_OP_AND) {
    uint64_t uint1 = (uint64_t)num1, uint2 = (uint64_t)num2;
    uint64_t result = uint1 & uint2;
    return (double)result;
  } else if(operator_type == TT_OP_RSH) {
    uint64_t uint1 = (uint64_t)num1, uint2 = (uint64_t)num2;
    uint64_t result = uint1 >> uint2;
    return (double)result;
  } else if(operator_type == TT_OP_LSH) {
    uint64_t uint1 = (uint64_t)num1, uint2 = (uint64_t)num2;
    uint64_t result = uint1 << uint2;
    return (double)result;
  }
...
SftError* eval_x_is_close_paren(Sft* self) {
  ...
  while(1) {
    ...
    Token* head = Stack_getHead(operator_cellar);

    if(head->type == TT_SY_LPAR) {
      if(head->func) {
        bool found_match = false;

        for(size_t i = 0; i < (sizeof(FN_LOOKUP) / sizeof(FN_LOOKUP[0])); ++i) {

          const Function* function = &FN_LOOKUP[i];
          ...
          if(!strcmp(function->name, head->func)) {
            found_match = 1;

            if((comas_encountered + 1) != function->args_req) {
              sprintf(self->error.message,
            ...
            if(Stack_getCount(number_cellar) < function->args_req) {
              sprintf(self->error.message,
            ...
            double fn_args[function->args_req];

            for(size_t i = 0; i < function->args_req; ++i) {
              double* num = Stack_pop(number_cellar);
              if(!num) {
                free(num);
            ...
            double result = function->ptr(fn_args, function->args_req);
...
SftError* Sft_evalTokens(Sft* self, TokenArray* tokens, double* out_result) {
  ...
  for(size_t i = 0; i < tokens->count; ++i) {
    ...
    Token token = tokens->tokens[i]; // The sigsev source
